...
if st.button("Calculate"):
    if Sex=="ชาย":
        if Sex == "ชาย" and Final < 19:
            st.info(f"{Final:.1f} น้อยกว่า 19 ")
            ...
            st.info (f"{Final:.1f} อยู่ระหว่าง 19.0 - 24.9")
            ...
            st.info(f"{Final:.1f} อยู่ระหว่าง 25.0 - 29.9")
            st.success(f"นํ้าหนักอยู่ในเกณฑ์อ้วน")
            st.warning(f"ภาวะแทรกซ้อน : ภาวะนํ้านหนักเกินมาก")
            st.image("S__13738045_0.jpg")

        else :
            if Final >=30.0:
                st.error(f"{Final:.1f} มากกว่า 30")
        ...
        if Final < 18:
            st.info(f"{Final:.1f} น้อยกว่า 18 ")
            st.success(f"นํ้าหนักตํ่ากว่าเกณฑ์ ")
            st.image("S__13738047_0.jpg")

        elif Final >=18.0 and Final <=23.9:
            st.info (f"{Final:.1f} อยู่ระหว่าง 18.0 - 23.9")
            st.success(f"นํ้าหนักสมส่วน")
            st.warning(f"ภาวะแทรกซ้อน : ภาวะนํ้าหนักเกินระยะเริ่มต้น")
            ...
            st.success(f"นํ้าหนักอยู่ในเกณฑ์อ้วน")
            st.warning(f"ภาวะแทรกซ้อน : ภาวะนํ้าหนักเกินมาก")
